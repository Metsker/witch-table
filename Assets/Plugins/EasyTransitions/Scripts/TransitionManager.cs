...
namespace EasyTransition
{
    ...
    {        
        ...
        public async UniTask<GameObject> TransitionIn(TransitionSettings transitionSettings, float delay = 0)
        {
        ...
        public async UniTask TransitionOut(Transition template, TransitionSettings transitionSettings, float delay = 0)
        {
            onTransitionCutPointReached?.Invoke();

            template.OnSceneLoad(SceneManager.GetActiveScene(), LoadSceneMode.Single);

            await UniTask.WaitForSeconds(transitionSettings.destroyTime);
